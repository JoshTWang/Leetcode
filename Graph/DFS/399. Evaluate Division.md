...
class Solution {
    ...
    public double[] calcEquation(List<List<String>> equations, double[] values, List<List<String>> queries) {
        ...
        // build the graph Map<dividend, <divisor, value>>
        for (int i = 0; i < equations.size(); i++) {
            List<String> equation = equations.get(i);
            String dividend = equation.get(0); // dividend / divison = quotient
...
* In this problem, actually we don't need to use backtracking. Because we can immediately return the answer if we find a path and we do not need to iterate all the possible paths from the start to the end.
