...
#### Too complicated with two crazy map
...
class LFUCache {
    ...
    private int minf; // min frequency
    ...
    /*  We do three things in get():
        1. Delete from freqncies.get(frequncy) LinkedHashSet
        2. Update frequencies and cache map using insert()
        3. Update minf if possible
    */
    public int get(int key) {
        ...
        keys.remove(key); // current key should move to next frequency set
    ...
    public void put(int key, int value) {
        ...
        if (frequencyAndValue != null) {
            ...
            get(key); // we should not call insert, otherwise we can't update minf, so we use get(key)
        } else {
            // insert and (maybe) delete
            if (capacity == cache.size()) {
                final Set<Integer> keys = frequencies.get(minf);
                final int keyToDelete = keys.iterator().next(); // linkedHashSet
                cache.remove(keyToDelete);
                keys.remove(keyToDelete);
            }
            minf = 1; // reset min freq since new value comes
            insert(key, 1, value);
        }
    }
}
